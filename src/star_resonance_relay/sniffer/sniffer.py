...
from star_resonance_relay.sniffer import scapy as sniffer_scapy
from star_resonance_relay.sniffer.scapy import BPSRPacket
...
class BPSRSniffer:
    ...
    def handle_packet(self, packet: Packet) -> None:
        ...
        if BPSRPacket not in packet:
            if connection not in sniffer_scapy.KNOWN_SERVERS:
                ...
                if self._is_server(payload):
                    ...
                    sniffer_scapy.KNOWN_SERVERS.add(connection)

            return

        try:
            logger.info(f"{connection.source.ip} <-> {connection.destination.ip}: {packet}")
            bpsr_packet = packet[BPSRPacket]

            for msg in self._processor.process_bytes(bytes(bpsr_packet)):
                match msg:
